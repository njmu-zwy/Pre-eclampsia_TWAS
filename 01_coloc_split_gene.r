...
COLOC_P3 <- 1E-05
#WIN_PLOT <- 2E05
gene_file <- fread2(paste0('/public/home/biostat06/TWAS/49_different_tissues/',opt$tissue,'_gene.txt'),header=F)[,1]
gene_file <- gene_file[which(str_detect(gene_file,'ENSG'))]
...
summary_file <- list(summ1 = '/public/home/biostat06/TWAS/Preeclampsia_gwas_summary/outcome.txt', 
                     type1 = 'cc',
                     #summ2 = paste0('/public/home/biostat06/TWAS/49_different_tissues/',opt$tissue,'/',opt$gene,'.tsv'), 
                     type2 = 'quant'
...
eqtl_title <- fread2('/public/home/biostat06/TWAS/49_different_tissues/molecular_trait_id.tsv',header=F)[1,] %>%
  unlist() %>%
  as.vector()

coloc_test <- plyr::alply(gene_file,1,function(gene){
  cat(paste0(gene,' \n'))
  summary_file$summ2 <- paste0('/public/home/biostat06/TWAS/49_different_tissues/',opt$tissue,'/',gene,'.tsv')
  eqtl_file <- fread2(summary_file$summ2)
  
  colnames(eqtl_file) <- eqtl_title
  if ( unique(eqtl_file$chromosome) %in% c(1:22) | nrow(eqtl_file) != 0 ) {
      eqtl_file <- eqtl_file[,c(3,5,6,9,10,18)] %>% na.omit()
      #gene <- opt$gene
      # intersect snp set
    ...
    if (nrow(gwas_file_sub)==0 | nrow(eqtl_file_sub)==0){
      cat(paste0('no snp in this gene','\n'))
    } else {
      # define dataset
      ...
      write.table(res,file=paste0('/public/home/biostat06/TWAS/COLOC_RESULT/coloc_result/',opt$tissue,'/',gene,'.txt'),
                  row.names=F,col.names=T,quote=F)
    }
  } else {
    cat(paste0('no snp in chromosome ',unique(eqtl_file$chromosome),' \n'))
  }
})
